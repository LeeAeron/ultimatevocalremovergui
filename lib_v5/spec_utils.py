...
def ensemble_inputs(audio_input, algorithm, is_normalization, wav_type_set, save_path, is_wave=False, is_array=False):
    ...
    if algorithm == AVERAGE:
        ...
        for i in range(len(audio_input)):  
            wave, samplerate = librosa.load(audio_input[i], sr=44100, mono=False)
            wavs_.append(wave)
...
def change_pitch_semitones(y, sr, semitone_shift):
    ...
    y_pitch_tuned = []
    for y_channel in y:
        y_pitch_tuned.append( librosa.resample(y_channel, orig_sr=sr, target_sr=int(sr*factor), res_type=wav_resolution_float_resampling) )
...
def average_audio(audio):
    ...
    for i in range(len(audio)):
        wave, sr = librosa.load(audio[i], sr=44100, mono=False)
        waves.append(wave[0])
